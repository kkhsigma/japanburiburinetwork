...
const riskConfig: Record<RiskLevel, { label: string; color: string; bg: string; border: string; glow: string; sort: number }> = {
  illegal:  { label: "違法",     color: "text-red-600",     bg: "bg-red-500/10",     border: "border-red-400/30",     glow: "shadow-red-500/5",    sort: 0 },
  high:     { label: "高リスク", color: "text-orange-600",  bg: "bg-orange-500/10",  border: "border-orange-400/30",  glow: "shadow-orange-500/5", sort: 1 },
  medium:   { label: "要注意",   color: "text-amber-600",   bg: "bg-amber-500/10",   border: "border-amber-400/30",   glow: "shadow-amber-500/5",  sort: 2 },
  low:      { label: "低リスク", color: "text-sky-600",     bg: "bg-sky-500/10",     border: "border-sky-400/30",     glow: "shadow-sky-500/5",    sort: 3 },
  safe:     { label: "合法",     color: "text-emerald-600", bg: "bg-emerald-500/10", border: "border-emerald-400/30", glow: "shadow-emerald-500/5",sort: 4 },
};
...
export function FloatingSubstances() {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      for (const card of cardsRef.current) {
        ...
        card.y = Math.min(Math.max(HEADER_HEIGHT + CARD_H / 2, card.y), r.height - CARD_H / 2);
      }
    };
    ...
    const tick = (now: number) => {
      if (filterRef.current.mode !== "none") {
  ...
  const getGridPositions = useCallback(
    (mode: FilterMode, value: FilterValue) => {
      ...
      const unmatchedCols = Math.max(1, Math.floor((cw - 40) / (CARD_W + 8)));
      const unmatchedGridW = unmatchedCols * (CARD_W + 8) - 8;
      const unmatchedStartX = (cw - unmatchedGridW) / 2 + CARD_W / 2;

      unmatched.forEach((compound, i) => {
        const col = i % unmatchedCols;
        const row = Math.floor(i / unmatchedCols);
        pos[compound.id] = {
          x: unmatchedStartX + col * (CARD_W + 8),
  ...
  const currentPositions = isFiltered
    ? getGridPositions(filterMode, filterValue!)
    : positions;
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isActive
                      ...
                      : `bg-white/60 text-gray-500 border-black/[0.06] hover:border-black/[0.12] hover:text-gray-700`
                    }
                  `}
                >
                  {cfg.label}
      ...
      {mounted && (
        <AnimatePresence>
          {mockCompounds.map((compound) => {
            ...
            const isMatch = compoundMatches(compound, filterMode, filterValue);
            ...
            return (
              <motion.div
                ...
                animate={{
                  ...
                  scale: isDimmed ? 0.88 : 1,
                }}
                transition={
                  isFiltered
                    ? { type: "spring", stiffness: 150, damping: 22, mass: 0.8 }
                    : { duration: 0.033, ease: "linear" }
                }
              ...
              >
                <Link
                  ...
                  className={`
                    ...
                    hover:bg-white hover:shadow-md ${risk.glow}
                    ${risk.border}
                    cursor-pointer
                  `}
                >
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[13px] font-bold font-mono text-gray-900 tracking-tight">
                      {compound.name}
                    </span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${risk.bg} ${risk.color} tracking-wide`}>
                      {risk.label}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {compound.chemical_family && (
                      <span className="text-[8px] text-gray-400 font-mono truncate max-w-[70px]">
                        {compound.chemical_family}
                      </span>
                    )}
                  </div>
